...
with profile(activities=[ProfilerActivity.CPU], record_shapes=True, with_modules=True) as prof:
    ...
    # Train the weights
    for train_step in range(start_step + 1, start_step + 4):
        # Generate a new image
        with record_function('sample_prep'):
            img = get_sample_img(train_loader, color=True)

        # Reset grid and optimizer
        with record_function('reset_things'):
            grid.reset()
            optimizer.zero_grad()
        ...
        with record_function('tensorboarding'):
        ...
        with record_function('backprop_and_reset'):
            with record_function('zero_grad'):
                optimizer.zero_grad()
            # Backprop
            with record_function('loss_backwards'):

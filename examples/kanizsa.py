...
# Load the image as grayscale and threshold it
kanizsa = cv2.imread('kanizsa.png', 0)
kanizsa = 1.0 - cv2.resize(kanizsa, (grid.shape[0]-2*10, grid.shape[1]-2*10))/255.0
kanizsa = (kanizsa > 0.5).astype(np.float32)
cv2.imshow('frame', kanizsa)
cv2.waitKey(0)
...
for i in range(kanizsa.shape[0]):
    for j in range(kanizsa.shape[1]):
        if(kanizsa[i,j] > 0):
            grid[10+i,10+j,0] = fdtd.PointSource(
                period = WAVELENGTH / SPEED_LIGHT
            )

# grid[10,10,0] = fdtd.PointSource(
#     period=WAVELENGTH / SPEED_LIGHT
# )

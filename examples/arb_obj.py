...
import torch
grid[:, :, :] = fdtd.LearnableAnisotropicObject(permittivity=1.0, is_substrate=True, name="cc_substrate")
...
#yl, xl = grid.objects[0].shape[0], grid.objects[0].shape[1]
iimg = torch.ones(9, 161, 161)
iimg[:, midpoint_y:midpoint_y + size, midpoint_x:midpoint_x + size] = 100
